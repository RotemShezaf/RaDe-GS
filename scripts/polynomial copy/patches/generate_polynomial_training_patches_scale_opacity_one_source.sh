#!/bin/bash
#
# Generate training patches (xyz + scale + opacity, one source) for all polynomial surfaces
#
# Identical to generate_polynomial_training_patches_scale_opacity.sh but uses the
# *_scale_opacity_one_source dataset configs, which set num_sources=1. This produces
...
#   DataSets/configs/polynomial/paraboloid_all_scale_opacity_one_source.yaml
#   DataSets/configs/polynomial/saddle_all_scale_opacity_one_source.yaml
#   DataSets/configs/polynomial/hyperbolic_paraboloid_all_scale_opacity_one_source.yaml
#
# USAGE:
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_scale_opacity_one_source.sh [options]
#
# OPTIONS:
...
#
# EXAMPLES:
#   # Generate all polynomial datasets with scale+opacity and one source (parallel)
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_scale_opacity_one_source.sh
#
#   # Generate only Paraboloid with more iterations
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_scale_opacity_one_source.sh --surfaces Paraboloid --num_iterations 5000
#
#   # Dry run
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_scale_opacity_one_source.sh --dry_run
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Generate training patches (xyz+scale+opacity, one source) for polynomial surfaces."
...
# Map surface names to scale+opacity one_source config files
declare -A SURFACE_CONFIGS
SURFACE_CONFIGS["Paraboloid"]="$CONFIG_DIR/paraboloid_all_scale_opacity_one_source.yaml"
SURFACE_CONFIGS["Saddle"]="$CONFIG_DIR/saddle_all_scale_opacity_one_source.yaml"
SURFACE_CONFIGS["HyperbolicParaboloid"]="$CONFIG_DIR/hyperbolic_paraboloid_all_scale_opacity_one_source.yaml"
...
# ============================================================================
echo "============================================================"
echo "Generate Polynomial Training Patches (xyz + scale + opacity, one source)"
echo "============================================================"
echo ""
echo "Configuration:"
...
echo ""
echo "To load the combined one-source scale+opacity dataset:"
echo "  from DataSets.gaussian_dataset import CombinedGaussianPatchDataset"
echo "  dataset = CombinedGaussianPatchDataset("
echo "      config='DataSets/configs/polynomial/combined_polynomial_all_scale_opacity_one_source.yaml',"
