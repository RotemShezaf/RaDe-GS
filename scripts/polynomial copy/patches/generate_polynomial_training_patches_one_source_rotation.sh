#!/bin/bash
#
# Generate training patches (one source, rotation) for all polynomial surfaces
#
# Identical to generate_polynomial_training_patches_one_source.sh but uses
# the *_one_source_rotation dataset configs, which include rotation in
# the attributes list alongside xyz.
#
# Dataset configs used:
#   DataSets/configs/polynomial/new_dataset/paraboloid_all_one_source_rotation.yaml
#   DataSets/configs/polynomial/new_dataset/saddle_all_one_source_rotation.yaml
#   DataSets/configs/polynomial/new_dataset/hyperbolic_paraboloid_all_one_source_rotation.yaml
#
# USAGE:
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_one_source_rotation.sh [options]
#
# OPTIONS:
...
#
# EXAMPLES:
#   # Generate all polynomial datasets with one source + rotation (parallel)
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_one_source_rotation.sh
#
#   # Generate only Paraboloid with more iterations
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_one_source_rotation.sh --surfaces Paraboloid --num_iterations 5000
#
#   # Dry run
#   ./scripts/polynomial/patches/generate_polynomial_training_patches_one_source_rotation.sh --dry_run
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Generate training patches (one source, rotation) for polynomial surfaces."
...
# Map surface names to one_source_rotation config files
declare -A SURFACE_CONFIGS
SURFACE_CONFIGS["Paraboloid"]="$CONFIG_DIR/new_dataset/paraboloid_all_one_source_rotation.yaml"
SURFACE_CONFIGS["Saddle"]="$CONFIG_DIR/new_dataset/saddle_all_one_source_rotation.yaml"
SURFACE_CONFIGS["HyperbolicParaboloid"]="$CONFIG_DIR/new_dataset/hyperbolic_paraboloid_all_one_source_rotation.yaml"
...
# ============================================================================
echo "============================================================"
echo "Generate Polynomial Training Patches (one source, rotation)"
echo "============================================================"
echo ""
echo "Configuration:"
echo "  Surfaces:    ${SURFACE_ARRAY[*]}"
echo "  Config dir:  $CONFIG_DIR"
echo "  Attributes:  xyz + rotation"
...
echo ""
echo "To load the combined one-source rotation dataset:"
echo "  from DataSets.gaussian_dataset import CombinedGaussianPatchDataset"
echo "  dataset = CombinedGaussianPatchDataset("
echo "      config='DataSets/configs/polynomial/new_dataset/combined_polynomial_all_one_source_rotation.yaml',"
echo "      attributes=['xyz', 'rotation'], ring=3"
echo "  )"
echo ""
echo "Done!"
