#!/usr/bin/env python3
"""Generate a PDF benchmark report with formatted tables from benchmark results.

Usage:
    python scripts/tosca/benchmark/generate_report_pdf.py \
        --benchmark_dir output/benchmarks/tosca_params/cat0_blue_high_res

Produces: <benchmark_dir>/benchmark_report.pdf
"""
...
def main():
    ...
    pdf_path = args.output or str(benchmark_dir / "benchmark_report.pdf")
    print(f"Generating PDF report with {len(rows)} runs...")

    with PdfPages(pdf_path) as pdf:
        # Page 1: Overall table sorted by Chamfer distance
        fig = make_table_figure(rows, "All Runs — Sorted by Chamfer Distance",
                                sort_key="chamfer_distance", ascending=True)
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # Page 2: Table sorted by PSNR (descending)
        fig = make_table_figure(rows, "All Runs — Sorted by PSNR (higher is better)",
                                sort_key="test_psnr", ascending=False)
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # Page 3: Table sorted by G→Surf mean(d²) (ascending = lower is better)
