...
def _masked_l1(pred, target, mask):
    """L1 loss averaged over masked (foreground) pixels only."""
    if mask is None:
        return torch.abs(pred - target).mean()
    # mask shape: (1, H, W), pred/target: (C, H, W)
    error = torch.abs(pred - target) * mask
    return error.sum() / (mask.sum() * pred.shape[0] + 1e-6)
...
def L1_loss_appearance(image, gt_image, gaussians, view_idx, mask=None):
    ...
    if app_model is GaussianModel.App_model.NO:
        return _masked_l1(image, gt_image, mask)
    ...
    if app_model is GaussianModel.App_model.GS:
        ...
        return _masked_l1(transformed, gt_image, mask)
    ...
    if app_model is GaussianModel.App_model.GOF:
        ...
        crop_mask = mask[:, top : top + H, left : left + W] if mask is not None else None
        ...
        return _masked_l1(transformed, crop_gt, crop_mask)
